"""
Train CNN classifier on ESC-50 dataset with 5-fold cross-validation.

Usage:
    python train_cnn.py --data_dir /path/to/ESC-50 --output_dir models/
"""
...
def train_fold(args, fold, device):
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(checkpoint, output_dir / f'cnn_esc50_fold{fold}.pt')
...
def main():
    parser = argparse.ArgumentParser(description='Train CNN classifier with 5-fold CV')
    ...
    # Save results
    with open(output_dir / 'cnn_5fold_results.json', 'w') as f:
        json.dump({
            'results': all_results,
