...
class ESC50(AudioDataset):
    base_folder = ''  # 修改：直接使用 root 目录，不需要子文件夹
    url = ""
    ...
    def download(self):
        # Check if dataset already exists (audio and meta directories)
        audio_path = Path(self.root) / self.audio_dir
        meta_path = Path(self.root) / 'meta'

        if audio_path.exists() and meta_path.exists():
            # Dataset already exists, skip download
            return

        # Check if we have a URL to download from
        if not self.url:
            raise RuntimeError(f"Dataset not found at {self.root} and no download URL provided. "
                             f"Please manually download the dataset or provide the correct path.")
